...
- (void)commonInit
{
    ...
    _menuViewContainer.userInteractionEnabled = YES;
    
    _contentViewContainer = [[UIView alloc] init];
    _contentViewContainer.userInteractionEnabled = YES;
    ...
    _leftMenuViewVisibleWidth = screen_width-main_vc_right_space;
    ...
    _menuOpacityViewRightBackgroundColor = [UIColor blackColor];
}
...
//    
    ...
    self.menuViewContainer.alpha = 0.5;
...
{
//    NSLog(@"当前点击的时啥玩意儿-----------%@",NSStringFromClass(touch.view.class)); //可以用这个方法来判断自己点中的控件使什么类型，然后用下面的if语句处理
